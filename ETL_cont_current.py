...
def get_TWN(prov, region, readings):
    ...
    while True:
        try:   
            response = requests.get(url.format(province_dict[prov], str(TWN_region_code).zfill(4))).json()
            break
        except json.decoder.JSONDecodeError:
            print("For some reason the JSON response was bad. Retrying this code...")
    ...
    try:
        ...
        print("bad region code?")
        print("JSON response we got from the " "region code: \n{}".format(response)) 
...
while True:
    while True:
        if datetime.datetime.now().minute in [0, 15, 30, 45]:
            break
        time.sleep(1)
    loaded_current_db = pd.read_csv("/Users/Alex/Coding/weather_db/Data/cont_current_db.csv")
