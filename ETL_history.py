...
def get_TWN(prov, region, readings):
    ...
    url = 'https://www.theweathernetwork.com/api/historical/ca{}{}/{}/{}/C/metric'.format(
        province_dict[prov], str(
            TWN_region_code).zfill(4), yesterday.month, yesterday.year)
    ...
    try:
        ...
        if fc_response['rdata']['year'] is '':
            loggr.info('TWN code {} does not have historical data for this' \
            ...
            for i in range(len(TWN_translation)):
                try:
                    TWN_data[i] = float(
                        fc_response['rdata'][TWN_translation[i]])
                except ValueError:
                    loggr.info('TWN code {} returned value of "{}" for' \
                        'reading "{}"'.format(str(TWN_region_code).zfill(4),
                                             TWN_data[i], TWN_translation[i]))
                    TWN_data[i] = np.nan
    except KeyError:
        loggr.critical('TWN code {} is a bad region code? JSON response' \
...
def get_EC(prov, region, readings):
    ...
    hist_link = soup.find('summary', {
        'id': 'yesterday'}).find_next_sibling().find(
    ...
    hist_year = int(soup.find('div', {'id': 'dynamicDataTable'}).find(
        'caption').get_text()[-4:])

    EC_data = [0 for _ in range(len(readings))]
    if hist_year == yesterday.year:
        try:
            table_row = soup.find('abbr', text=str("%02d" % (
                yesterday.day,))).find_all_next('td')
        except AttributeError:
            loggr.info('EC code {} does not currently have data for '\
        ...
        for i, j in zip(range(3), [0, 1, 7]):
            try:
                ...
                if table_row[j].get_text() == 'LegendMM':
                    loggr.info('EC code {} reports a missing value for '\
                        'reading "{}"'.format(EC_region_code, readings[i]))
                    pass
                else:
                    loggr.warning('EC code {} returned value of "{}" for '\
                        'reading "{}"'.format(
                        EC_region_code, table_row[j].get_text(),
                        readings[i]))
                    pass
                EC_data[i] = np.nan
        return EC_data
    else:
        loggr.info('EC code {} only has historical data for '\
            'year {}'.format(EC_region_code, hist_year))
        return [np.nan] * 3


history_db = pd.read_csv('{}/Data/history_db.csv'.format(PATH))
if history_db['date'].iloc[-1] == (now.date() - datetime.timedelta(
        days=1)).strftime('%Y-%m-%d'):
    loggr.warning('Data already collected for today. Process terminated.')
    pass
else:
    no_of_regions = len(region_codes['TWN_region_code'])
    loggr.info('starting to extract history for 1st of '\
    ...
    for j in range(no_of_regions):
        ...
        for op in (get_TWN, get_EC):
            data = op(
                region_codes.province[j], region_codes.region[j], readings)
            provider_history[i] = pd.DataFrame({
                'date': datetime.datetime.now().date() - datetime.timedelta(
                    days=1),
