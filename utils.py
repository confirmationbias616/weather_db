...
def erase_today():
	for filename, day in zip(
		['forecast_db', 'history_db'], [today, yesterday]):
...
def ETL_regions():
	...
	#province_dict has EC province labels as keys and TWN province labels as values
	province_dict = {
	    'ns':'nova-scotia',
	    ...
	    for EC_region_code in range(1,500):
	        ...
	        if response.status_code == 200:
	            ...
	            try:
	                ...
	                EC_region_name = soup.find('h1', {'property': 'name'}).get_text()[:-5]
	                regions = regions.append({'province':province_dict[EC_province],'region':EC_region_name,'EC_region_code':EC_region_code},ignore_index=True)
	                #print('province: {}'.format(province_dict[EC_province]))
	                #print('region code: {}'.format(EC_region_code))
	                #print('region name: {}'.format(EC_region_name))
	                #print('\n')
	        ...
	        if empty_code > 10:
	            #print('moving on to the next province')
	            #print('\n')
	...
	#fucntion that will be used to apply row by row on the EC regions dataframe to fill in TWN codes
	def find_TWN_code(EC_info):
	    ...
	    if '(' in EC_name:
	        return np.nan
	    TWN_name = EC_name #creating TWN copy of region name so we can manipulate to make relevant to TWN nomenclature
	    ...
	    if response.status_code == 200:
	        html = response.content
	        soup = BeautifulSoup(html, 'html.parser')
	        TWN_region_code = soup.find('div', {'id': 'prebid', 'class': 'module'}).find_next('script').find_next('script').find_next('script').get_text().split(';')[1][-5:-1]
	...
	regions['TWN_region_code'] = regions['TWN_region_code'].apply(lambda x: int_or_nan(x))

	regions.dropna(axis=0, inplace=True)

	regions['TWN_region_code'] = regions['TWN_region_code'].apply(lambda x: x[:-2]).apply(int).apply(str).apply(lambda x: x.zfill(4))
...
def geocode():
	...
	regions = pd.read_csv('{}/Data/region_codes.csv'.format(PATH)).drop('Unnamed: 0',axis=1)

	regions['prov_region'] = regions['province'].map(str) + '_' + regions['region']

	# for testing purposes only:
	#regions = regions.head(10)
